...
if SelectedID = -10 then Exit;
  ...
  begin
    ShowMessage('Вы должны быть авторизированы для совершения этого действия.');
    Exit;
  end;

  if CurrentUser.PowerLevel <> 2 then
  begin
    ShowMessage('У вас недостаточно прав для совершения этого действия.');
    Exit;
  end;
...
if CurrentUser.PowerLevel <> 2 then
begin
  ShowMessage('Необходимо обладать правами администратора для совершения этого действия!');
  Exit;
end;

      if MessageDlg('Вы уверены, что хотите удалить эту запись?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    begin
      try
...
begin
  ...
  ComboBoxAdminMenuStatus.Items.AddObject('Заблокирован', TObject(2));
